#!/usr/bin/env python3
"""
Best-of: Extract travel + expense info from OCR'd documents stored in a JSONL file.

Outputs:
- out/docs_extracted.jsonl
- out/docs_extracted.csv
- out/trips_aggregated.csv
"""
...
def run_extract(
    ...
    try:
        ...
        print(f"  [Warning] JSON extraction failed: {exc}")
        data = {
            "source_file_summary": {"doc_type": "unknown", "category": "other"},
            "trip": {},
            "costs": {},
            "passenger_info": {},
            "evidence": [{"field": "error", "quote": str(exc)[:200]}],
        }
...
def main() -> int:
    ...
    tokenizer, model = load_model_4bit(args.model)
    print("✅ Model loaded")
    ...
    for i, row in enumerate(docs, 1):
        ...
        try:
            data = run_extract(
                ...
                model,
                text,
                src_file or f"doc_{i}",
                max_new_tokens=args.max_new_tokens,
            )
            print(
                f"  ✅ Extracted: {data.get('source_file_summary', {}).get('doc_type', 'unknown')}"
            )
            ...
            data = {
                ...
                "costs": {},
                "passenger_info": {},
                "evidence": [{"field": "error", "quote": str(exc)}],
            }
    ...
    for d in extracted:
        ...
        total = to_float(costs.get("total_amount")) or sum(
            filter(
                None,
                [
    ...
    df_agg["has_trip_info"] = df_agg[
        ["trip_start_date", "trip_end_date", "destination"]
    ].notna().any(axis=1)
    df_ok = df_agg[df_agg["has_trip_info"]].copy()

    if len(df_ok) > 0:
        grp = df_ok.groupby(
            ["trip_start_date", "trip_end_date", "destination"], dropna=False
        )
        df_trips = grp.agg(
