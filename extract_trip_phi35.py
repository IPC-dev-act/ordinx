...
OPTIMIZED FOR: microsoft/Phi-3.5-mini-instruct

Phi-3.5-mini-instruct is excellent for structured data extraction tasks:
- 3.8B parameters (smaller than Qwen 2.5-3B)
- Better instruction following
- Strong JSON generation capabilities
- Fast inference on consumer GPUs (4GB VRAM in 4-bit)
...
def build_messages(doc_text: str, source_file: str) -> List[Dict[str, str]]:
    ...
    Optimized for Phi-3.5-mini-instruct.
    """
    system = (
...
# ----------------------------
def load_phi_4bit(model_path: str):
    ...
    bnb = BitsAndBytesConfig(
        ...
        bnb_4bit_use_double_quant=True,  # Better for Phi models
    )
...
def run_extract(tokenizer, model, doc_text: str, source_file: str, max_new_tokens: int = 600) -> Dict[str, Any]:
    ...
    # Phi-3.5 uses standard chat template
    ...
    out = model.generate(
        **inputs,
        ...
        do_sample=False,
        temperature=None,  # Use greedy decoding for Phi
        top_p=None,
...
def main():
    ...
    ap.add_argument("--model", default="microsoft/Phi-3.5-mini-instruct", help="Model name or path (default: Phi-3.5-mini-instruct)")
    ...
    print(f"🧠 Loading Phi-3.5-mini-instruct: {args.model}")
    tokenizer, model = load_phi_4bit(args.model)
    print("✅ Model loaded")
